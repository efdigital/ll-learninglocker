 ...
 */
schema.statics.limitIdsByOrg = async function limitIdsByOrg(
  stepIds,
  cumulativeIds,
  level = 0
) {
  if (level > 0) {
    try {
      const orgs = await Organisation.find({ parent: { $in: stepIds } });
      const childIds = map(orgs, '_id');
      return await Organisation.limitIdsByOrg(
        childIds,
...
schema.post('deleteOne', { document: true, query: false }, async function(orgModel) {
