...
schema.methods.createResetToken = async function createResetToken(expires = 24) {
  const token = await bcrypt.hash(this.email, 8);
  ...
  // return the token and expiry as an object
  // this is the format that resetTokens are stored in the user document
  return { token, expires: expiresDate };
};
 ...
 * @param  {Function} next
 */
const preSavePasswordCheck = async (user, next) => {
...
schema.pre('validate', async function handlePreSave(next) {
  const user = this;
  try {
    const checkedUser = await checkNewUser(user);
    ...
    if (user.isNew) {
      const org = await Organisation.findOne({ _id: user.ownerOrganisation });
...
schema.pre('save', async function handlePostSave(next) {
  const user = this;
  await preSavePasswordCheck(user, next);
});
