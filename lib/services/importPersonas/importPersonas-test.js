...
describe('import personas service', () => {
  ...
  before(async () => {
    // Clean up any lingering queue subscriptions from previous test runs (e.g. worker tests)
    try {
      await unsubscribeAll();
      console.log('Successfully cleaned up queue subscriptions before import personas tests');
    } catch (err) {
      console.warn('Failed to clean up queue subscriptions before import personas tests:', err.message);
    }
  });
